...
$gatunek ="SELECT * FROM GATUNKI order by NAZWA";
$jezyk ="SELECT * FROM JEZYKI order by NAZWA";
$utwor = "SELECT * FROM UTWORY where ID=$idu";
$query = mysql_query($gatunek);
$query2= mysql_query($jezyk);

if($op){

$query3= mysql_query($utwor);
$nuta = mysql_fetch_object($query3);


$ID_g=$nuta->ID_GATUNKI;
$ID_j=$nuta->ID_JEZYKI;
}

echo "<h2><table border=\"10\"><form name=\"OK\" method=\"post\" action=\"edit_add.php?idp=$idp&idu=$idu&op=$op\">";


echo '<tr><td>Tytul utworu:</td> <td><input type="text" name="tytul_utworu" value="';
if($op){echo $nuta->TYTUL;}
echo '"></td></tr>';




echo '<tr><td>Gatunek:</td> <td>
<select name="gatunek">';

echo '<option value="0"></option>';
while ($gatunek = mysql_fetch_object($query)){

if($ID_g==$gatunek->ID){
echo '<option selected value="'.$gatunek->ID.'">'.$gatunek->NAZWA."</option>\n";
}
else{
echo '<option value="'.$gatunek->ID.'">'.$gatunek->NAZWA."</option>\n";
}
}
echo '</select></td></tr>';

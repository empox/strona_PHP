...
<?php     
	...
	$licz=count($spis);
	if ($_SESSION['log']){
	echo "<h1>Wykonawcy&nbsp&nbsp <a href=\"formWyk.php?op=0\"><img src=\"dodaj.png\" ></a></h1> ";
	}
	else{
	echo "<h1>Wykonawcy&nbsp&nbsp </h1> ";
	}
	echo '<br/>';

	echo "<form method=\"post\" action=\"wykonawcy.php\">";
...
for($i=0;$i<=$licz-1;$i++){
	...
	echo "<a href=\"wykonawcy.php?pl=$spis[$i]\">".$spis[$i]." - </a>";
...
$pl=$_GET['pl'];
echo "<a href=\"wykonawcy.php?pl=9\">[0-9]</a>";
echo '</p><br/>';
if($pl==9){
	$select1="SELECT ID,NAZWA FROM WYKONAWCY where NAZWA  <'A'";
}
else{
	$select1="SELECT ID,NAZWA FROM WYKONAWCY where NAZWA LIKE '$pl%'";
}
echo '<br/>';
...
$select="SELECT ID,NAZWA FROM WYKONAWCY where UPPER(NAZWA) LIKE UPPER('%$dana%')";
...
if ($_SESSION['log']){
echo "<tr><td><h4>".$wiersz->NAZWA."</h4></td><td><h4><a href=\"usuwanieWyk.php?idw=$wiersz->ID&op=0\">&nbsp&nbsp<img src=\"usun2m.png\"></a></h4</td> <td><h4><a href=\"formWyk.php?idw=$wiersz->ID&op=1\">&nbsp&nbsp<img src=\"popraw.png\"></a></h4></td></tr>";
}
else{
echo "<tr><td><h4>".$wiersz->NAZWA."</h4></td></tr>";
...
echo "<table>";
while ($wiersz = mysql_fetch_object($query1)){
if ($_SESSION['log']){
echo "<tr><td><h4>".$wiersz->NAZWA."</h4></td>  <td><h4><a href=\"usuwanieWyk.php?idw=$wiersz->ID&op=0\">&nbsp&nbsp<img src=\"usun2m.png\"></a></h4</td> <td><h4><a href=\"formWyk.php?idw=$wiersz->ID&op=1\">&nbsp&nbsp<img src=\"popraw.png\"></a></h4></td></tr>\n\r";
}
else{

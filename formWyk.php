   ...
   <div class="content_spacer">
	...
	<!-- ==================================== srodek ===========================-->
      <?php
		...
		$op=$_GET['op'];
	  if(!$op){
	  echo "<h1>Dodawanie wykonawcy do bazy</h1>";
	  }
		$kr = "select * from KRAJE order by NAZWA";
		$query1 = mysql_query($kr);
		
	   
	  $select = "SELECT * FROM WYKONAWCY where ID = ".$idw;
	  ...
	  if ($op)
		...
		$ID_k=$wiersz->ID_KRAJE;
		
		echo "<h1>Edycja wykonawcy: <br><br><a href=\"wykonawcy.php\">".$wiersz->NAZWA."</a></h1>";
		}
		
		
		
		echo "<h2><table><form name=\"OK\" method=\"post\" action=\"edit_addWyk.php?op=$op&idw=$idw\">";
		echo '<tr><td>Nazwa wykonawcy:</td> <td><input type="text" name="nazwa" value="';
		if($op){echo $wiersz->NAZWA;}
		echo '"></td></tr>';
		
		echo '<tr><td>Kraj: </td><td><select name="kraj">';

echo '<option value="0"></option>';
while ($kraj = mysql_fetch_object($query1)){

if($ID_k==$kraj->ID){
echo '<option selected value="'.$kraj->ID.'">'.$kraj->NAZWA."</option>\n";
}
else{
...
echo "</td></tr></select>";
		...
		echo "<tr><td><input type=\"submit\" value=\"ZAPISZ\"></form><form method=\"post\" action=\"wykonawcy.php\"><input type=\"submit\" value=\"ANULUJ\"></td></tr>";
		echo "</form></table>";

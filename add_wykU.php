...
include 'polaczenie.php';
 $IDP = $_GET['idp'];
 $_SESSION['ID_UTWORU'] = $_GET['idu'];
 $select2 = "SELECT TYTUL FROM UTWORY WHERE ID=".$_SESSION['ID_UTWORU'];
 $query2 = mysql_query($select2);
 $wiersz2  = mysql_fetch_object($query2);
 $_SESSION['UTWORY_TYTUL'] = $wiersz2->TYTUL;
 $FORMULARZ = "add_wykU.php?idp=".$IDP."&idu=".$_SESSION['ID_UTWORU'];


echo "<h1>Dodawanie artysty do utworu: <a href=\"spisU.php?idp=$IDP\"><br><br>".$_SESSION['UTWORY_TYTUL']." </a></h1>";

?>


<form action=<?php echo $FORMULARZ; ?> method="POST">
<h4>Wyszukaj artystę: <input type="text" name="tekst">  <input type="Submit" value="Szukaj"></h4> 
</form><br>
<?php
$IDU = $_GET['idu'];
$s = $_POST['tekst'];
...
if ($b != 1)
{
	$select = "SELECT * FROM WYKONAWCY WHERE UPPER(NAZWA) LIKE UPPER('%".$s."%') ORDER BY NAZWA";
	$query = mysql_query($select);
	while($wiersz  = mysql_fetch_object($query)) {
	echo "<h4><a href =\"wybRole.php?IDP=".$IDP."&IDW=".$wiersz->ID,"\">".$wiersz->NAZWA."</a></h4>"."\n";
	}
